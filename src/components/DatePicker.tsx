...
export const DatePicker: React.FC<DatePickerProps> = ({ selectedDate, onSelectDate, isAdmin = false }) => {
  ...
  const moveWeek = (days: number) => {
    const date = new Date(startDate); date.setDate(date.getDate() + days);
    setStartDate(days < 0 && date < new Date(new Date().setHours(0, 0, 0, 0)) ? new Date() : date);
  };

  return <div className="w-full">
    <div className="mb-3 flex items-center justify-between"><span className="text-sm font-bold capitalize text-[#5D4037]">{startDate.toLocaleDateString('es-ES', { month: 'long' })} {startDate.getFullYear()}</span><div className="flex gap-2"><button type="button" onClick={() => moveWeek(-7)} disabled={isPrevDisabled} className={cn('rounded-full border p-2 transition-colors', isPrevDisabled ? 'cursor-not-allowed border-gray-200 text-gray-300' : 'border-[#E5D3B3] text-[#5D4037] hover:bg-[#E5D3B3]/20')}><ChevronLeft className="h-4 w-4" /></button><button type="button" onClick={() => moveWeek(7)} className="rounded-full border border-[#E5D3B3] p-2 text-[#5D4037] transition-colors hover:bg-[#E5D3B3]/20"><ChevronRight className="h-4 w-4" /></button></div></div>
    <div className="grid grid-cols-7 gap-1.5 sm:gap-2">{dates.map((date) => {
      const dateStr = formatDateKey(date); const selected = selectedDate === dateStr; const blocked = blockedDates.has(dateStr); const disabled = date.getDay() === 0 || (blocked && !canSelectBlocked);
