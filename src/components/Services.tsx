      ...
      <div className="container mx-auto px-6">
        <div className="text-center max-w-4xl mx-auto mb-20">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <span className="text-[#8D6E63] font-medium tracking-[0.4em] uppercase text-xs mb-4 block">Experiencia Premium</span>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif text-[#5D4037] mb-6 leading-tight">Nuestros Servicios de Belleza</h1>
            <p className="text-[#5D4037]/70 font-light leading-relaxed text-lg mb-10 max-w-2xl mx-auto">Descubre tratamientos diseñados para realzar tu belleza natural y brindarte un momento de desconexión total.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <Link to="/reserva" className="bg-[#5D4037] text-white px-8 py-4 rounded-full text-sm font-bold uppercase tracking-widest hover:bg-[#4a332c] transition-all shadow-lg w-full sm:w-auto">Reservar cita</Link>
              <a href="https://wa.me/593969272530" target="_blank" rel="noopener noreferrer" className="bg-white text-[#5D4037] border border-[#E5D3B3] px-8 py-4 rounded-full text-sm font-bold uppercase tracking-widest hover:bg-[#FAF9F6] transition-all w-full sm:w-auto">Contactar por WhatsApp</a>
            </div>
          </motion.div>
        </div>
        ...
        <div id="servicios-anchor" className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-32">
          {displayCategories.map((category, index) => {
            ...
            return (
              <motion.div key={category.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="group flex flex-col">
                <div className="relative h-[400px] rounded-3xl overflow-hidden mb-6 shadow-2xl">
                  <img src={image} alt={`Servicios de ${category.nombre}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" referrerPolicy="no-referrer" />
                  ...
                  <div className="absolute bottom-8 left-8 right-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {categoryServices.slice(0, 4).map((service) => <span key={service.id} className="text-[8px] uppercase tracking-widest text-white/60 border border-white/20 px-2 py-1 rounded-full">{service.nombre}</span>)}
                      {!categoryServices.length && <span className="text-[8px] uppercase tracking-widest text-white/60 border border-white/20 px-2 py-1 rounded-full">Próximamente</span>}
                    </div>
                  </div>
                </div>
                <p className="text-sm text-[#5D4037]/70 font-light leading-relaxed px-4 mb-6 flex-grow">Descubre nuestros servicios exclusivos de {category.nombre.toLowerCase()} diseñados para tu bienestar.</p>
                <div className="px-4 mt-auto"><Link to={linkPath} className="inline-block w-full text-center py-3 rounded-full border-2 border-[#5D4037] text-[#5D4037] font-bold text-xs uppercase tracking-widest hover:bg-[#5D4037] hover:text-white transition-colors">Ver Servicios</Link></div>
              </motion.div>
            );
          })}
        </div>
        ...
        <div className="bg-[#FAF9F6] rounded-[3rem] p-8 md:p-16 text-center">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-16">
            {[{ number: '+100', label: 'Clientas Atendidas' }, { number: '+5', label: 'Años de Experiencia' }, { number: '100%', label: 'Atención Profesional' }, { number: '5.0', label: 'Resultados de Calidad' }].map((stat) => (
              <div key={stat.label}><div className="text-3xl md:text-4xl font-serif text-[#5D4037] font-bold mb-2">{stat.number}</div><div className="text-[10px] uppercase tracking-widest text-[#5D4037]/60 font-bold">{stat.label}</div></div>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 text-left">
            {[{ name: 'María Fernanda', text: 'La atención es increíble y los resultados en mi cabello fueron espectaculares.' }, { name: 'Andrea Gómez', text: 'El ambiente es súper relajante y el personal muy profesional.' }, { name: 'Carla Ruiz', text: 'Los masajes relajantes son de otro mundo. Salí renovada.' }].map((testimonial) => (
              <div key={testimonial.name} className="bg-white p-8 rounded-3xl shadow-sm border border-[#E5D3B3]/20"><div className="flex text-[#E5D3B3] mb-4">{[...Array(5)].map((_, index) => <Star key={index} className="w-4 h-4 fill-current" />)}</div><p className="text-[#5D4037]/70 italic mb-6 text-sm leading-relaxed">"{testimonial.text}"</p><div className="font-bold text-[#5D4037] text-sm uppercase tracking-wider">{testimonial.name}</div></div>
            ))}
          </div>
        </div>
      </div>
