...
type MissingField = "login" | "services" | "name" | "phone" | "email" | "schedule" | "attendance";
...
export default function ReservationForm() {
  ...
  const [missingField, setMissingField] = React.useState<MissingField | null>(null);
  ...
  const updateField = (field: keyof typeof formData, value: string) => {
    ...
    if ((field === "nombre" && missingField === "name") || (field === "whatsapp" && missingField === "phone") || (field === "email" && missingField === "email")) setMissingField(null);
  };
  const guideToField = (field: MissingField, message: string) => {
    setMissingField(field);
    toast.error(message, { description: "Te llevamos al dato que necesitas completar.", duration: 5500 });
    window.setTimeout(() => {
      const target = document.getElementById(`reservation-${field}`);
  ...
  const validateReservation = () => {
    if (!user) return guideToField("login", "Inicia sesión con Google para reservar");
    if (!selectedServices.length) return guideToField("services", "Selecciona al menos un servicio");
    if (formData.nombre.trim().length < 2) return guideToField("name", "Escribe el nombre de la persona");
    if (formData.whatsapp.replace(/\D/g, "").length < 10) return guideToField("phone", "Escribe un número de teléfono válido");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email.trim())) return guideToField("email", "Escribe un correo electrónico válido");
    if (!selectedTime) return guideToField("schedule", "Selecciona una fecha y un horario disponible");
    if (!confirmedAttendance) return guideToField("attendance", "Confirma que asistirás a tu cita");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!authUser) return void guideToField("login", "Tu sesión expiró. Vuelve a ingresar con Google.");
    ...
    if ((currentBlocks ?? []).some((block) => block.hora === null || normalizeTime(block.hora) === normalizeTime(selectedTime!))) { setSelectedTime(null); guideToField("schedule", "Ese día u horario fue bloqueado. Selecciona otra opción."); return; }
    ...
    toast.success("Reserva enviada correctamente"); clearSelection(); setSelectedTime(null); setConfirmedAttendance(false); setMissingField(null); setFormData({ nombre: user?.user_metadata?.full_name || "", whatsapp: "", email: user?.email || "", notas: "" });
  };
  ...
  return <motion.section id="reserva" className="min-h-screen bg-[#FAF9F6] px-4 pb-16 pt-24 sm:px-6 md:pt-28" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
    <div className="container mx-auto max-w-6xl"><div className="mb-7 flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><span className="mb-2 block text-[10px] font-bold uppercase tracking-[0.3em] text-[#8D6E63]">Bolsa de Bienestar</span><h1 className="font-serif text-3xl leading-tight text-[#5D4037] md:text-4xl lg:text-5xl">Finaliza tu experiencia Marobel</h1></div><Link to="/servicios" className="text-[10px] font-bold uppercase tracking-widest text-[#5D4037] hover:text-[#8D6E63]">Seguir eligiendo servicios</Link></div>
      {!user && <Card id="reservation-login" tabIndex={-1} className={`mb-6 rounded-2xl border-none bg-white shadow-md outline-none ${missingField === "login" ? "ring-2 ring-amber-500 ring-offset-4" : ""}`}><CardContent className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between"><div><h2 className="mb-1 font-serif text-xl text-[#5D4037]">Ingresa para reservar</h2><p className="text-xs text-[#5D4037]/60">Necesitamos tu cuenta para guardar y confirmar la cita.</p></div><Button onClick={login} className="h-10 rounded-full bg-[#5D4037] px-6 text-[10px] font-bold uppercase tracking-widest text-white">Ingresar con Google</Button></CardContent></Card>}
      <form onSubmit={handleSubmit} className="grid items-start gap-5 lg:grid-cols-[1.25fr_0.75fr]"><div className="space-y-5">
        <Card id="reservation-services" tabIndex={-1} className={`rounded-2xl border-none bg-white shadow-md outline-none ${missingField === "services" ? "ring-2 ring-amber-500 ring-offset-4" : ""}`}><CardContent className="space-y-4 p-5 md:p-6"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#E5D3B3]/25"><Sparkles className="h-4 w-4 text-[#8D6E63]" /></div><div><h2 className="font-serif text-xl text-[#5D4037]">Servicios seleccionados</h2><p className="text-xs text-[#5D4037]/55">Puedes quitar servicios antes de reservar.</p></div></div>{!selectedServices.length ? <div className="rounded-xl border border-dashed border-[#E5D3B3] bg-[#FAF9F6] p-6 text-center"><p className="mb-4 text-xs text-[#5D4037]/65">Todavía no has seleccionado servicios.</p><Link to="/servicios" className="inline-flex rounded-full bg-[#5D4037] px-6 py-2.5 text-[10px] font-bold uppercase tracking-widest text-white">Ver servicios</Link></div> : <div className="space-y-3">{selectedServices.map((service: any) => <div key={service.id} className="flex items-center justify-between gap-3 rounded-xl border border-[#E5D3B3]/30 bg-[#FAF9F6] p-3"><div><p className="font-bold text-[#5D4037]">{service.name}</p><p className="text-[10px] uppercase tracking-widest text-[#5D4037]/55">{service.category} {service.duration ? `- ${service.duration}` : ""}</p></div><div className="flex items-center gap-3"><span className="font-bold text-[#8D6E63]">${Number(service.price).toFixed(2)}</span><button type="button" onClick={() => removeService(service.id)} className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-red-500"><Trash2 className="h-4 w-4" /></button></div></div>)}</div>}</CardContent></Card>
        <Card className="rounded-2xl border-none bg-white shadow-md"><CardContent className="space-y-4 p-5 md:p-6"><h2 className="font-serif text-xl text-[#5D4037]">Datos de la persona</h2><div className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label className="text-[10px] font-bold uppercase tracking-widest text-[#5D4037]/65">Nombre</Label><div className="relative"><User className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#5D4037]/30" /><Input id="reservation-name" aria-invalid={missingField === "name"} value={formData.nombre} onChange={(e) => updateField("nombre", e.target.value)} className={`h-11 rounded-xl bg-[#FAF9F6] pl-11 ${missingField === "name" ? "border-2 border-amber-500" : "border-none"}`} placeholder="Nombre completo" /></div></div><div className="space-y-2"><Label className="text-[10px] font-bold uppercase tracking-widest text-[#5D4037]/65">Teléfono</Label><div className="relative"><MessageCircle className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#5D4037]/30" /><Input id="reservation-phone" inputMode="tel" aria-invalid={missingField === "phone"} value={formData.whatsapp} onChange={(e) => updateField("whatsapp", e.target.value)} className={`h-11 rounded-xl bg-[#FAF9F6] pl-11 ${missingField === "phone" ? "border-2 border-amber-500" : "border-none"}`} placeholder="WhatsApp" /></div></div><div className="space-y-2 md:col-span-2"><Label className="text-[10px] font-bold uppercase tracking-widest text-[#5D4037]/65">Correo</Label><div className="relative"><Mail className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#5D4037]/30" /><Input id="reservation-email" type="email" aria-invalid={missingField === "email"} value={formData.email} onChange={(e) => updateField("email", e.target.value)} className={`h-11 rounded-xl bg-[#FAF9F6] pl-11 ${missingField === "email" ? "border-2 border-amber-500" : "border-none"}`} placeholder="correo@ejemplo.com" /></div></div></div></CardContent></Card>
        <Card id="reservation-schedule" tabIndex={-1} className={`rounded-2xl border-none bg-white shadow-md outline-none ${missingField === "schedule" ? "ring-2 ring-amber-500 ring-offset-4" : ""}`}><CardContent className="space-y-4 p-5 md:p-6"><h2 className="font-serif text-xl text-[#5D4037]">Fecha, horario y profesional</h2><div className="space-y-2"><Label className="text-[10px] font-bold uppercase tracking-widest text-[#5D4037]/65">Profesional</Label><select value={selectedProfessional} onChange={(e) => setSelectedProfessional(e.target.value)} className="h-11 w-full rounded-xl bg-[#FAF9F6] px-4 text-sm text-[#5D4037] outline-none">{professionalsList.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div><DatePicker selectedDate={selectedDate} onSelectDate={setSelectedDate} /><Calendar selectedDate={selectedDate} selectedTime={selectedTime} onSelectSlot={(time) => { setSelectedTime(time); setMissingField(null); }} totalDuration={totalDuration} selectedProfessional={selectedProfessional} /><div className="space-y-2"><Label className="text-[10px] font-bold uppercase tracking-widest text-[#5D4037]/65">Notas opcionales</Label><textarea value={formData.notas} onChange={(e) => updateField("notas", e.target.value)} className="min-h-[84px] w-full resize-none rounded-xl border-none bg-[#FAF9F6] p-3 text-sm outline-none" /></div></CardContent></Card>
      </div><aside className="space-y-5 lg:sticky lg:top-24">
        <Card className="rounded-2xl border-none bg-white shadow-md"><CardContent className="space-y-4 p-5"><h2 className="font-serif text-xl text-[#5D4037]">Recomendaciones</h2>{!recommendedServices.length ? <p className="text-sm text-[#5D4037]/60">Tu selección ya está completa.</p> : recommendedServices.map((service) => <div key={service.id} className="flex items-center justify-between gap-3 rounded-xl border border-[#E5D3B3]/25 p-3"><div><p className="text-sm font-bold text-[#5D4037]">{service.nombre}</p><p className="text-xs text-[#8D6E63]">${Number(service.precio || 0).toFixed(2)}</p></div><Button type="button" variant="outline" className="rounded-full text-[10px]" onClick={() => { addService({ id: service.id, name: service.nombre, price: Number(service.precio || 0), category: service.categoria, description: service.descripcion, duration: service.duracion }); setMissingField(null); }}>Agregar</Button></div>)}</CardContent></Card>
